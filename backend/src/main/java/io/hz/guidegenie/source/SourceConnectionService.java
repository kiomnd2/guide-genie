...
public class SourceConnectionService {
    ...
    public SourceConnection create(Long projectId, SourceType type, Map<String, Object> config,
        ...
        SourceConnection connection = new SourceConnection(
            projectId, type, config, tokenCipher.encrypt(rawToken));
        SourceConnection saved = connectionRepository.save(connection);

        // 연결 검증 후 초기 전체 수집 트리거(비동기)
        connectorRegistry.forType(type).testConnection(saved);
        syncService.sync(saved.getId(), true);
        return saved;
    }
