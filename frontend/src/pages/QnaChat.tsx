...
import { getToken, type Citation } from '../api/client'
...
export default function QnaChat() {
  ...
  const ask = async () => {
    ...
    const res = await fetch(`/api/projects/${projectId}/qna`, {
      method: 'POST',
      headers: {
  ...
  return (
    <div>
      <h2>Q&amp;A</h2>
      <div className="card">
        <input placeholder="프로젝트 ID" value={projectId} onChange={(e) => setProjectId(e.target.value)} />
      </div>
